...
class MinesweeperAI():
    ...
    def make_probabilistic_random_move(self):
        unvisited_cells = set(itertools.product(range(self.height), range(self.width))) - self.moves_made - self.mines

        if not unvisited_cells:
            return None

        # Calculate the number of marked mines and mines left
        marked_mines = len(self.mines)
        mines_left = self.total_mines - marked_mines

        # Calculate probabilities for each unvisited cell
        probabilities = {}
        for cell in unvisited_cells:
            ...
            if total_count > 0:
                probability = mines / total_count
            else:
                # If there's no information about a cell, assume uniform probability
                probability = mines_left / len(unvisited_cells)
